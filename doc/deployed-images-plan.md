...
1. **즉시(A)**: 로컬 static/image 를 백엔드 static 볼륨으로 rsync → 전 게임 이미지 라이브(오늘 해결).
2. **단기(C 일부)**: 크롤러를 서버에서 정기 실행하도록 스케줄(신규/갱신분이 서버 static에 바로 쌓이게). genshin 중단 원인 조사.
3. **장기(B)**: R2로 이전해 머신 종속 제거(권장 최종 형태).
...
> **즉시책 = 로컬 static→백엔드 rsync**, **근본책 = 이미지 R2(오브젝트 스토리지)로 이전해 머신 종속 제거.**
