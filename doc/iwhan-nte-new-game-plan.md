...
## 1. 데이터 소스 — everness.info/ko + 공식 홈페이지 두 곳만 (확정)
> 이 두 사이트만 사용한다(namu/fandom/커뮤니티 등 외부 소스는 쓰지 않음).
- **주 소스: `https://everness.info/ko`** (이환 전용 한글 DB). 캐릭터 목록 `/ko/characters`, 상세 `/ko/character/<slug>`(추정) 200. → 캐릭터 데이터(이름/희귀도/속성/무기/스킬/아이콘)의 1차 출처.
- **보조 소스: 공식 홈페이지 `https://nte.perfectworld.com/kr`** — everness에 없는 공식 이미지/캐릭터 소개·로고·이벤트/공지 등을 보완. (캐릭터 일러/공식 아트가 필요하면 여기서.)
...
## 5. 주의/리스크
- **everness.info 추출 방식**이 최대 변수(RSC 스트리밍) — RSC 파싱/내부 API/헤드리스 렌더 중 스파이크로 확정. everness에 없는 항목만 공식 홈페이지로 보완.
...
> **데이터 소스는 everness.info/ko(주) + 공식 홈페이지(보조) 두 곳만.** 관건은 everness.info의 RSC 추출 방식 스파이크. 이후 속성·무기 필터 아이콘·데이터 공백 추적 등 기존 인프라 그대로 재사용.
