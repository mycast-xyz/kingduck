...
| **3rd-party 팬DB `browndust2-db.souseha.com`** | **Vue SPA + Firebase + 자체 API** | **2026-06-21 스파이크 발견(아래 §4-1a).** 가장 데이터가 풍부하나 **법적·기술적 주의 필요.** |

#### 4-1a. `browndust2-db.souseha.com` 스파이크 결과 (2026-06-21)

사용자 제보 사이트. 직접 분석한 기술 구조:

- **프론트**: Vite로 빌드된 **Vue SPA**(`/assets/main-*.js`, Cloudflare). 서버 렌더 없음 → HTML 파싱 불가, JS 실행 또는 API 직접 호출 필요.
- **데이터 백엔드**: **Firebase(Firestore 추정, JS 번들에 firebase 47회)** + 자체 API **`https://api-browndust2-wiki.souseha.com`**. API 루트는 404 JSON 응답(라우트별로 동작).
- **보호**: **reCAPTCHA Enterprise + Firebase App Check 정황** → SchaleDB류 공개 정적 JSON과 달리 **단순 fetch로 안 열릴 가능성 높음**(App Check 토큰 요구).
- **이미지 CDN**: `https://img-bd2db.souseha.com/images/...`, 캐릭터 이미지 경로 예: `/characters/Lathel_1.webp`(영문 캐릭터 key 확인 가능 — 우리 `metadata` 매핑에 참고 가치).
- **범위**: characters·costumes(skins)·weapons·talents·materials·territory·tier-maker 등 **매우 포괄적**, 다국어(ko/en/ja/cn/tw), 전체 sitemap(`/sitemap.xml`) 제공.
- **운영자**: Patreon/Ko-fi/Discord 보유 개인/팀(souseha).

**판단**:
- ✅ **참고 가치 최상**: BD2 데이터 모델(어떤 필드·코스튬 분류·영문 캐릭터 key)이 어떻게 구성되는지 검증용으로 훌륭.
- ⚠️ **스크랩 소스로는 비권장(1순위 아님)**: (1) 타 팬사이트 DB를 그대로 긁어 우리 DB를 채우는 것은 **노력 전유·ToS 위반 소지**, (2) Firebase App Check/recaptcha로 **기술적 차단 가능성**, (3) 구조 변동 위험.
- **권장**: 데이터가 꼭 필요하면 **운영자에게 Discord/Ko-fi로 데이터 제공·제휴를 문의**하는 쪽이 정공법. 자동 스크랩 전에 반드시 사용자(프로젝트 오너) 판단·허가 필요.

**권장 스파이크 순서(갱신)**:
1. **공식 사이트** `browndust2.neowiz.com` 내부 API/JSON 발굴 — 라이선스 측면 가장 안전.
2. **팬덤 위키** `browndust2.fandom.com/api.php`(MediaWiki, 인증 불필요, CC 라이선스) — 구조 데이터.
3. `souseha` 팬DB는 **모델 참고용**으로만 활용하거나, 사용할 경우 **운영자 허가** 후.
4. 1~3 불충분 시 Chromium(puppeteer) 렌더 추출 — 최후 수단.

#### 4-1b. 2차 데이터소스 스파이크 결과 (2026-06-21) — **깨끗한 공개 소스 없음**

NTE의 everness.info(공개 GraphQL) 같은 깔끔한 소스를 BD2에서 찾지 못함. 실측 결과:

| 후보 | 결과 |
|------|------|
| **souseha API** `api-browndust2-wiki.souseha.com` | `/characters`가 "GitHub API error: 404"(text/plain) 반환 — GitHub 백킹이지만 깨짐. 라이브 사이트는 **Firebase Firestore + App Check**(`database`/`data-connect` 번들 확인)로 클라이언트 직접 로드 → **REST 미동작·스크랩 불가**. |
| **`jilv220/Brown-Dust-2-Wiki-Data`** (GitHub) | data/{characters,costumes,exclusive_gears,skills,talents} 구조는 좋으나 **archived·2025-02 마지막 push** → 1년+ stale, 폐기. |
| **공식** `www.browndust2.com` | **AmazonS3 정적 마케팅 사이트** — 구조화 데이터/API 없음. |
| **팬덤** `browndust2.fandom.com` / `bd2.fandom.com` | api.php 응답 없음(URL/차단 미상) — 추가 확인 필요하나 사용 시 wikitext 파싱 부담 큼. |

**결론**: BD2는 **NTE처럼 즉시 크롤 가능한 공개 구조화 API가 없다.** 현 시점 선택지:
- (A) **팬덤 위키 재조사 + wikitext 파싱 크롤러** — 합법(CC)·현행이나 파싱 비용 큼.
- (B) **souseha 운영자에게 데이터 제휴 문의**(Discord/Ko-fi) — 정공법이나 외부 의존.
- (C) **공식 게임 데이터마이닝**(에셋/번들) — 회색지대, 권장 안 함.
- (D) **BD2 데이터는 보류**, Phase 0 스캐폴딩 유지 후 소스 확보 시 재개.
→ **오너 결정 필요.** 다른 게임의 미노출 데이터 노출(doc/data-coverage-audit.md)이 더 값싸고 즉효라 우선순위 재검토 권장.
