<script lang="ts">
	...
	onMount(async () => {
		...
		if (editData) {
			...
			formData = {
				...
				officialLink: editData.officialLink || '',
	...
	async function handleSubmit() {
		...
		if (!formData.gameId || !formData.title || !formData.startTime || !formData.endTime) {
			toastStore.warning('필수 항목을 모두 입력해주세요.');
			return;
		}

		try {
			if (isEditing) {
				const response = await client.put(`/api/v0/admin/event/${eventId}`, formData);
				const code = response.data.resultCode;
				if (code !== 200) {
					toastStore.error(response.data.resultMsg || '수정에 실패했습니다.');
					return;
				}
				toastStore.success('이벤트가 수정되었습니다.');
			} else {
				const response = await client.post('/api/v0/admin/event', formData);
				const code = response.data.resultCode;
				if (code !== 200 && code !== 201) {
					toastStore.error(response.data.resultMsg || '추가에 실패했습니다.');
					return;
				}
				toastStore.success('이벤트가 추가되었습니다.');
			}
			// 저장 완료 후 모달 닫기 — watcher가 목록을 재갱신
			closeModal();
		} catch (error) {
			console.error('이벤트 저장 중 오류 발생:', error);
			toastStore.error('이벤트 저장에 실패했습니다.');
		}
	}
