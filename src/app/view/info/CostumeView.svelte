...
<Layer title={title || initData?.name || '코스튬'}>
	<div class="grid grid-cols-2 md:grid-cols-4 gap-4 p-4">
		{#each costumes as costume}
			<div
				class="group relative rounded-lg border border-gray-200 bg-white p-2 shadow-sm transition-all hover:shadow-md dark:border-gray-700 dark:bg-gray-800"
			>
		...
		{#if costumes.length === 0}
			<div class="col-span-full py-8 text-center text-gray-500">코스튬 정보가 없습니다.</div>
		{/if}
	</div>
</Layer>
