<script lang="ts">
	...
	let majorTraces = $derived(
		nodes.filter((node: any) => node.Desc || node.description || node.PropertyNodeDescribe)
	);
	let statTraces = $derived(
		nodes.filter((node: any) => !node.Desc && !node.description && !node.PropertyNodeDescribe)
	);
	...
	const getFormattedName = (item: any) => {
		...
		if (item.PropertyNodeTitle) return item.PropertyNodeTitle; // WW
