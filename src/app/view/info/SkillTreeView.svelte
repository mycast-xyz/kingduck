<script lang="ts">
	...
	const { listData, currentUrl, isMobile, initData, extraData } = $props<{
		listData: any;
		currentUrl: string;
		isMobile: boolean;
		initData: any;
	...
	let selectedList = $state(currentList?.[0] || null);
	let selectedLevel = $state(1);

	// 탭 변경 시 선택 초기화
	$effect(() => {
		selectedList = currentList?.[0] || null;
		selectedLevel = 1;
	});
        ...
        <div class="flex border-b border-gray-200 dark:border-gray-700 mb-4">
            {#each tabLabels as label, index}
                ...
                >
                    {label}
                    {#if activeTab === index}
                        <div class="absolute bottom-0 left-0 w-full h-0.5 bg-indigo-600 dark:bg-indigo-400"></div>
                    {/if}
                </button>
            {/each}
        </div>
    ...
    <div class="flex flex-col md:flex-row gap-4 p-4 max-h-64 overflow-y-auto">
        ...
        <div class="flex md:flex-col gap-2 overflow-x-auto md:overflow-y-auto md:w-1/4 md:border-r border-gray-200 dark:border-gray-700 pr-2">
            {#each currentList as item}
                ...
                >
                     <div class="w-10 h-10 flex-shrink-0 bg-gray-300 dark:bg-gray-600 rounded-full p-1 overflow-hidden">
