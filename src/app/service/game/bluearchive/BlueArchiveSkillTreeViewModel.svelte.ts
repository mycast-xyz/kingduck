...
/**
 * 블루 아카이브(Blue Archive) 스킬 뷰모델 — SkillTreeView가 다른 게임과 동일하게 렌더하도록 어댑팅.
 * SchaleDB에서 가공한 metadata.skills = [{ name, type, description, icon }]
 * (레벨 스케일 없음 → maxLevel 1). description은 줄바꿈(\n)을 <br>로 변환해 표시.
 */
export class BlueArchiveSkillTreeViewModel extends SkillTreeViewModel {
	get items(): SkillItem[] {
